    ...
    <>
      <div className="px-3 pt-5">
          ...
          >
            {fieldsHolidays?.map((field: any, i) => (
              <div className="grid grid-cols-7 pt-5 gap-3">
                <p>{field.name}</p>
                <p>
                  {moment(field.from_date).format("DD MMM")} -
                  {moment(field.to_date).format("DD MMM")}
                </p>

                <p>
                  {field.dine_in_checked == false ? (
                    <div className="flex items-center gap-2">
                      <ClearIcon className="h-2 w-2" /> Closed
                    </div>
                  ) : (
                    <>
                      {field.dine_in_from}-{field.dine_in_to}
                    </>
                  )}
                </p>
                <p>
                  {field.pickup_checked == false ? (
                    <div className="flex items-center gap-2">
                      <ClearIcon className="h-2 w-2" /> Closed
                    </div>
                  ) : (
                    <>
                      {field.pickup_from}-{field.pickup_to}
                    </>
                  )}
                </p>
                <p>
                  {field.delivery_checked == false ? (
                    <div className="flex items-center gap-2">
                      <ClearIcon className="h-2 w-2" /> Closed
                    </div>
                  ) : (
                    <>
                      {field.delivery_from}-{field.delivery_to}
                    </>
                  )}
                </p>
                <p>
                  {field.drive_thru_checked == false ? (
                    <div className="flex items-center gap-2">
                      <ClearIcon className="h-2 w-2" /> Closed
                    </div>
                  ) : (
                    <>
                      {field.drive_thru_from}-{field.drive_thru_to}
                    </>
                  )}
                </p>
                <p>
                  {field.delivery_partners_checked == false ? (
                    <div className="flex items-center gap-2">
                      <ClearIcon className="h-2 w-2" /> Closed
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      {field.delivery_partners_from}- {" "}
                      {field.delivery_partners_to}
                      <p>
                        <ClearIcon
        ...
        {fields.map((_, i) => (
          <div className="p-5 bg-popover rounded-lg mt-5">
            ...
            <div className="pb-3  border-b border-white">
              ...
              {watch(`single_holiday.${i}.to_date`) <
                watch(`single_holiday.${i}.from_date`) && (
                <div className="text-[10px] text-warn pt-2 ml-40">
            ...
            <div className="flex items-center gap-3 mt-5">
              <Switch
                checked={Types?.map((type) =>
                  watch(`single_holiday.${i}.${type?.value}_checked`)
                )
                  ?.map((item) => item == true)
                  .includes(true)}
                onCheckedChange={(e) => {
            ...
            {Types?.map((type) => (
              <div className="grid  grid-cols-3 mt-3 ">
                <div className="flex gap-3 items-center">
                  <Switch
                    checked={watch(
                      `single_holiday.${i}.${type?.value}_checked`
                    )}
                    disabled={
                      watch(`single_holiday.${i}.${type?.value}_checked`) ==
                      false
        ...
        <div className="flex justify-end">
          <button
            className={`cursor-pointer text-right py-3 font-bold text-primary text-[16px] ${
              fields.length == 1 ? "text-primary-foreground" : ""
            }`}
