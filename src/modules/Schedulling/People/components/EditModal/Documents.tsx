...
const Documents = ({ resetFrom }: any) => {
  ...
  const { updateDocuments, isLoadingUpdateDocuments, editEmployee } =
    usePeopleHttp({
  ...
  const providedAndSign = documentsData?.data?.filter(
    (doc) => doc.type == 1 || doc.type === 3
  );
  const providedAndSignDoc = watch("documents")?.filter(
    (doc) => doc.type == 1 || doc.type === 3
  );
  const handleConfirm = () => {
    if (modalName == "reject") {
  ...
  return (
    <>
      ...
      <Label>
        Documents needed from {watch("first_name")} {watch("last_name")}
      </Label>
      {watch("documents")?.map((item: any, index) => (
        <>
          {[1, 3].includes(item?.type) && (
            <div className="flex flex-col gap-[10px] border-b border-input border-b-1 p-3">
              <div className="flex items-center justify-between">
                ...
                >
                  <CustomDropDown
                    ...
                    onValueChange={(e) => {
                      if (e == 1) {
                        const filterDocuments = watch("documents")?.filter(
                          (doc) => doc.id !== item.id
                        );
      ...
      {isOpen && (
        <div className="flex items-center gap-3">
          <CustomDropDown
            options={providedAndSign
              ?.filter(
                (doc) =>
                  !providedAndSignDoc?.some(
                    (providedDoc) => providedDoc.id === doc.id
                  )
              )
              ?.map((document) => ({
                label: document.name,
            ...
            onValueChange={(e) => {
              append({
                id: e,
                name: documentsData?.data?.find((item: any) => item?.id == e)
                  ?.name,
                type: documentsData?.data?.find((item: any) => item?.id == e)
                  ?.type,
...

      {
        ...
        >
          <div
            className=" my-[16px] text-primary border-[2px]  border-dashed w-full text-center p-[17px] rounded-[4px] border-[#D1D5D7] cursor-pointer"
            onClick={() => {
              setIsOpen(true);
            }}
          >
      ...
      <Label>
        Uploaded by
        {watch("branches")?.find((item: any) => item?.is_home == true)?.name}
      </Label>

      {watch("documents")?.map((item: any, index) => (
        <>
          {[2].includes(item?.type) && (
            <div className="flex flex-col gap-[10px] border-b border-input border-b-1 p-3">
              <div className="flex items-center justify-between">
                <p>{item?.name}</p>
                <CustomDropDown
      ...
      <CustomModal
        ...
        confirmbtnStyle={
          modalName == "reject" ? "" : " text-success border-success"
        }
      />
    </>
  );
