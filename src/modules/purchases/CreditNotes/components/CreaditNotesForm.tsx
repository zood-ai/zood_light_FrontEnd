...
const CreaditNotesForm = ({
  ...
  const { receiveCreditNotice, isLoadingReceive } = useCreditNotesHttp({});
  return (
    <div className="pb-20">
      {/* invoice info */}
      <div className="bg-popover px-3 py-4 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <FormControl onClick={() => {}}>
            ...
            >
              <span>{invoiceData?.business_date || ""}</span>
              <CalendarIcon
                className="ml-auto h-4 w-4 opacity-50 shrink-0"
                color="#D4E2ED"
              />
      ...
      {/* credit notes */}
      {invoiceData?.creditNotices?.length > 0 && (
        <div className="bg-warn-foreground border border-warn rounded-[8px] p-2 mt-2 mx-2">
          ...
          <div className="flex flex-col gap-2">
            {invoiceData?.creditNotices?.map((credit: CreditNote) => (
                ...
                <div className="flex gap-3">
                  {/* <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setCreditIds((prev) => [...prev, credit.id]);
                      } else {
                        setCreditIds((prev) =>
                  ...
                  <Label className="flex  flex-col gap-2 text-textPrimary">
                    ...
                    <span className="text-warn text-[12px] font-medium">
                      {/* Order: SAR 11.50, Invoice: SAR 12.50 Qty:2 */}
                      {credit?.note !== "none" && credit?.note}
                    </span>
                  </Label>
                </div>
                <div className="flex items-center gap-6">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <Button
              disabled={!creditIds.length || isLoadingReceive}
              onClick={() => {
                receiveCreditNotice({ creditIds });
                handleCloseSheet();
              }}
              className="bg-[#F9F5F9] text-[#000] border border-gray-400"
            >
              Recieve
            </Button>
          </div>
      ...
      <div className="flex justify-between items-center px-2 py-8">
        ...
        <div className="flex flex-col gap-4 text-textPrimary">
          <span>SAR {invoiceData?.sub_total?.toFixed(2) || ""}</span>
          <span>SAR {invoiceData?.total_vat?.toFixed(2) || ""}</span>
          <span className=" font-semibold">
            SAR {invoiceData?.total?.toFixed(2) || ""}
          </span>
        </div>
      </div>
    </div>
