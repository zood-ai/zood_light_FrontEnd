...
export const AddNewItem: React.FC<CreateNewItemProps> = ({
  ...
  const {
    ...
    ["get/purchase-orders", filterObj, OrderOne, rowData],
    `forecast-console/orders/${OrderOne?.branch}/supplier/${OrderOne?.supplier_id}/items`,
    {},
    {},
    "post"
  );
  ...
  useEffect(() => {
    ...
    const unsharedItems = [
      // Items in arr that are not present in watchedItems with the same value and unit
      ...arr.filter(
        (item1) =>
          !watchedItems.some(
            (item2) =>
              item2.item_id === item1.value && item2.unit === item1.unit
          )
      ),
      // Items in watchedItems that are not present in arr with the same value and unit
      ...watchedItems.filter(
        (item2) =>
          !arr.some(
            (item1) =>
              item1.value === item2.item_id && item1.unit === item2.unit
          )
      ),
    ];
